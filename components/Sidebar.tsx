...
export default function Sidebar() {
  ...
  const setEnvironment = async (newEnv: AdminEnvironment) => {
    if (newEnv === env) return;
    if (newEnv === "PROD") {
      const ok = await confirm({
        title: PROD_SWITCH_CONFIRM.title,
        description: PROD_SWITCH_CONFIRM.description,
        message: (
          <>
            <p>{PROD_SWITCH_CONFIRM.message}</p>
            <p className="mt-3 font-semibold text-slate-800">
              {PROD_SWITCH_CONFIRM.question}
            </p>
          </>
        ),
        confirmLabel: PROD_SWITCH_CONFIRM.confirmLabel,
        cancelLabel: PROD_SWITCH_CONFIRM.cancelLabel,
        variant: "danger",
      });
      if (!ok) return;
    }

    setStoredAdminEnvironment(newEnv);
  ...
  const envLabels: Record<AdminEnvironment, string> = {
    DEV: "DEV",
    PROD: "PROD",
  };
  ...
  return (
    <aside className="z-30 flex h-screen w-[260px] shrink-0 flex-col border-r border-slate-200/80 bg-white shadow-sidebar">
      <div className="flex flex-col gap-1 p-5">
        <Link
          href="/"
          className="mb-6 flex items-center gap-3 rounded-xl p-1 transition-opacity hover:opacity-90"
        >
          <BrandLogo size="md" />
